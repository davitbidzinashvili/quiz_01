...
function filterExpensiveProducts(products, priceThreshold) {
  ...
  return products.filter(products => products.price>priceThreshold).map(products => products.name)
}

function applyDiscount(products, discountPercentage) {
  return products.map(products => products.price - (prodicts.price * discountPercentage))
}

function getAvailableProducts(products) {
   return products.filter(products => products.stock>0).map(products => [products.name,products.price])
}
